...
**Deferral by other tooling is conditional, and it now holds.** Loom wires its own `.loom/hooks/guard-destructive.sh`, which is a dispatcher: it execs this canonical guard only when that copy passes **both** a version probe and a capability probe for **Bash-tool write confinement** (grepping for the `worktree-write-confinement` marker). This guard implements that category and emits the marker, so in a Loom-managed repo the dispatcher runs *this* guard rather than Loom's vendored `guard-destructive-generic.sh`.

That was not true before 0.9.0, and the difference mattered: the capability probe is a single marker that switches which guard runs **entirely**, so satisfying it while behind on other categories would have silently downgraded protection. Emitting it was therefore gated on reaching behavioral parity with the vendored copy, verified by comparing both guards' deny/allow decisions over the full pattern surface rather than by counting features. The reconciliation closed a `git stash` gap (`pop`/`drop`/`clear` had **no** coverage here at all) and four checks that scanned the raw command instead of the literal-redacted working copy — which made this guard deny ordinary prose that merely mentioned a destructive statement. One deliberate difference remains: `aws iam delete-role` is a hard **deny** here and an ask in the vendored copy, so a Loom-managed repo gets stricter behavior on IAM deletion after the switch, not weaker.
